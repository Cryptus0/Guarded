...
module.exports = {
  callback: async (client, interaction) => {
    ...
    if (targetUserRolePosition >= botRolePosition) {
      const botMissingPermsEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "There was a problem with kicking that user. They have the same or higher role than me. Contact a server administrator to fix this."
        )
        .setFooter({ text: "Guarded" });
    ...
    try {
      ...
      await interaction.editReply({ embeds: [kickSuccessEmbed] });
    } catch (error) {
      const errorEmbed = new EmbedBuilder()
        .setColor(0xe03a3a)
        .setTitle("🤖 Error Kicking User 🤖")
        .setDescription(
          "There was an error kicking that user. Please try again later."
        )
        .setFooter({ text: "Guarded" });
  ...
  options: [
    {
      name: "user",
      description: "Who do I need to kick?",
      required: true,
      type: ApplicationCommandOptionType.User,
    },
    {
      name: "reason",
      description: "Why should I kick this person?",
      required: false,
